...
module GoldenChip_apex4_045pObf_s ();
...
apex4_045pObf_s ObfuscatedModule (.sk(sk), .i_0_(inputs[0]), .i_1_(inputs[1]), .i_2_(inputs[2]), .i_3_(inputs[3]), .i_4_(inputs[4]), .i_5_(inputs[5]), .i_6_(inputs[6]), .i_7_(inputs[7]), .i_8_(inputs[8]), 
...
initial begin
	rk = $fopen("RNG1.txt", "r");
	f = $fopen("functionalMismatchapex4_045pObf_s.txt", "w");
end
initial begin
	for(i = 0; i < MAX_COUNT; i = i +1) begin
		for(l = 0; l < 10; l = l + 1) begin
			...
			keyPercentageCorrect = keyPercentageCorrect / 100;
