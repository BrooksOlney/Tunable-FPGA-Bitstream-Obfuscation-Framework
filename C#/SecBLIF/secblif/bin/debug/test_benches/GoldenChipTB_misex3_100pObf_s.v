...
module GoldenChip_misex3_100pObf_s ();
...
misex3_100pObf_s ObfuscatedModule (.sk(sk), .a(inputs[0]), .b(inputs[1]), .c(inputs[2]), .d(inputs[3]), .e(inputs[4]), .f(inputs[5]), .g(inputs[6]), .h(inputs[7]), .i(inputs[8]), .j(inputs[9]), .k(inputs[10]), .l(inputs[11]), .m(inputs[12]), .n(inputs[13]), 
...
initial begin
	rk = $fopen("RNG1.txt", "r");
	f = $fopen("functionalMismatchmisex3_100pObf_s.txt", "w");
end
initial begin
	for(i = 0; i < MAX_COUNT; i = i +1) begin
		for(l = 0; l < 10; l = l + 1) begin
			...
			for(k = 0; k < 367; k = k + 1) begin
				keyPercentageCorrect = keyPercentageCorrect + sk[k];
			end
			keyPercentageCorrect = keyPercentageCorrect / 367;
