...
module GoldenChip_spla_qmap_060pObf_s ();
...
spla_qmap_060pObf_s ObfuscatedModule (.sk(sk), .i_0_(inputs[0]), .i_1_(inputs[1]), .i_8_(inputs[2]), .i_6_(inputs[3]), .i_7_(inputs[4]), .i_9_(inputs[5]), .i_10_(inputs[6]), .i_11_(inputs[7]), .i_15_(inputs[8]), .i_12_(inputs[9]), .i_13_(inputs[10]), .i_14_(inputs[11]), .i_4_(inputs[12]), .i_3_(inputs[13]), .i_5_(inputs[14]), .i_2_(inputs[15]), 
...
initial begin
	rk = $fopen("RNG1.txt", "r");
	f = $fopen("functionalMismatchspla_qmap_060pObf_s.txt", "w");
end
initial begin
	for(i = 0; i < MAX_COUNT; i = i +1) begin
		for(l = 0; l < 10; l = l + 1) begin
			...
			for(k = 0; k < 378; k = k + 1) begin
				keyPercentageCorrect = keyPercentageCorrect + sk[k];
			end
			keyPercentageCorrect = keyPercentageCorrect / 378;
